    ...
    def generar_mapa(self, ancho:int, alto:int):
        ...
        for y in range(alto):
            for x in range(ancho):
                ...
                self.grid_altura[y][x] = self._generar_ruido.noise3(x, y, 0)

                self.grid_humedad[y][x] = self._generar_ruido.noise3(x + 1000, y + 1000, 1)

                self.grid_temperatura[y][x] = self._generar_ruido.noise3(x + 2000, y + 2000, 2)
...
class MapaProcedural:
    ...
    def dibujar(self, pantalla):
        ancho_mapa = self.ancho * self.tamaño_casilla
        alto_mapa = self.alto * self.tamaño_casilla
        ...
        for y in range(self.alto):
            for x in range(self.ancho):
                casilla = self.casillas[y][x]
                pos_x = x * self.tamaño_casilla + offset_x
                pos_y = y * self.tamaño_casilla + offset_y
                
                ubicacion = (pos_x, pos_y)
                imagen = self.imagenes_bioma.get(casilla.bioma)

                if imagen:
                    pantalla.blit(imagen, ubicacion)
                else:
                    color = self.COLORES_BIOMA.get(casilla.bioma, (0, 0, 0))
                    pygame.draw.rect(pantalla, color, (pos_x, pos_y, self.tamaño_casilla, self.tamaño_casilla))

        for enemigo in self.enemigos:
            ex, ey = enemigo.current_node
            pos_x = ex * self.tamaño_casilla + offset_x + (self.tamaño_casilla // 2)
            pos_y = ey * self.tamaño_casilla + offset_y + (self.tamaño_casilla // 2)
            pygame.draw.circle(pantalla, (255, 50, 50), (pos_x, pos_y), self.tamaño_casilla // 3)
import numpy as np
